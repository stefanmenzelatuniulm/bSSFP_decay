...
function [C1 C2 C2s]=decayCoefficients(as,TR,f,n_tot,epsilon,T1max,Meq)
    ...
    a(:,:,:,1)=a(:,:,:,1)/2;
    TR=repmat(TR,[na 1 nf n_tot+1]); %indices: a, TR, f, pulse
    E1=exp(-TR/T1); %indices: a, TR, f, pulse
    E2=exp(-TR/T2); %indices: a, TR, f, pulse
    for k=1:nf
        E1(:,:,k,1)=E1(:,:,k,1).^f(k);
        E2(:,:,k,1)=E2(:,:,k,1).^f(k);
    end
    ...
    for k=2:n_tot+1
        ...
        disp("Calculating relaxation coefficients for pulse "+num2str(k)+" (not counting a/2 preparation pulse)");
